...
const cpp_1 = require("cpp"),
  ...
  SHIPPING = cpp_1.KuroApplication.IsBuildShipping() ? 1 : 0,
...
class LogAnalyzer {
    ...
    LogAnalyzer.z8 = TimerSystem_1.TimerSystem.Forever(
      (e) => {
        ...
        for (const R of LogAnalyzer.i9)
          R[1] > STATISTICS_THRESHOLD && L.push(R);
        if ((LogAnalyzer.i9.clear(), 0 < L.length)) {
          ...
          var n = {};
          for (const I of L) n[I[0].toString()] = I[1];
          LogAnalyzer.e9(
        ...
        for (const c of LogAnalyzer.o9) {
          var z = c[1];
          t < z ? _.push(c) : z > STATISTICS_THRESHOLD && y.push(c);
        }
        if (
          ...
          for (let e = 0; e < y.length; ++e) {
            var T = y[e];
            i[e] = { Msg: T[0], Count: T[1] };
          }
          LogAnalyzer.e9(
            "log_message_report",
            new LogReportRecord(LogAnalyzer.j8, O, i),
          );
        }
        LogAnalyzer.Z8.Stop();
      },
  ...
  static Q8() {
    ...
    LogAnalyzer.r9 = TimerSystem_1.TimerSystem.Forever((e) => {
      if (
        ...
        (a += 1),
        (LogAnalyzer.Zm = 0) !== LogAnalyzer.s9.size)
      ) {
        var o = {};
        let e = 0;
  ...
  static A5(r, a, L, n, g, A, l, t, _, y) {
    LogAnalyzer.a9.Start(), (LogAnalyzer.Y8[L] += 1);
