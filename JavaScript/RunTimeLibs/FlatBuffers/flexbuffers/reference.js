...
function valueForIndexWithKey(
  ...
  const _indirect = (0, reference_util_js_1.indirect)(
    dataView,
    offset,
    parentWidth,
  );
  ...
  return new Reference(
    dataView,
    elementOffset,
    (0, bit_width_util_js_1.fromByteWidth)(byteWidth),
    packedType,
    `${path}/${key}`,
  );
}
class Reference {
  ...
  isNumber() {
    return (
      (0, value_type_util_js_1.isNumber)(this.valueType) ||
      (0, value_type_util_js_1.isIndirectNumber)(this.valueType)
    );
  }
  isFloat() {
    return (
      value_type_js_1.ValueType.FLOAT === this.valueType ||
  ...
  isString() {
    return (
      value_type_js_1.ValueType.STRING === this.valueType ||
      value_type_js_1.ValueType.KEY === this.valueType
    );
  }
  isBool() {
  ...
  intValue() {
    ...
    if (this.valueType === value_type_js_1.ValueType.UINT) {
      return (0, reference_util_js_1.readUInt)(
        this.dataView,
        this.offset,
        this.parentWidth,
      );
    ...
    if (this.valueType === value_type_js_1.ValueType.INDIRECT_INT) {
      return (0, reference_util_js_1.readInt)(
        this.dataView,
        (0, reference_util_js_1.indirect)(
          this.dataView,
          this.offset,
          this.parentWidth,
        ),
        (0, bit_width_util_js_1.fromByteWidth)(this.byteWidth),
      );
    }
  ...
  stringValue() {
    if (
      this.valueType === value_type_js_1.ValueType.STRING ||
      ...
      return (0, flexbuffers_util_js_1.fromUTF8Array)(
  ...
  get(key) {
    ...
    if (
      ...
      if ((0, value_type_util_js_1.isTypedVector)(this.valueType)) {
        const _valueType = (0, value_type_util_js_1.typedVectorElementType)(
          this.valueType,
        );
        _packedType = (0, value_type_util_js_1.packedType)(
          _valueType,
          bit_width_js_1.BitWidth.WIDTH8,
        );
      } else if ((0, value_type_util_js_1.isFixedTypedVector)(this.valueType)) {
        const _valueType = (0,
        value_type_util_js_1.fixedTypedVectorElementType)(this.valueType);
        _packedType = (0, value_type_util_js_1.packedType)(
          _valueType,
          bit_width_js_1.BitWidth.WIDTH8,
        );
    ...
    if (typeof key === "string") {
      const index = (0, reference_util_js_1.keyIndex)(
        key,
        this.dataView,
        this.offset,
        this.parentWidth,
  ...
  length() {
    ...
    if ((0, value_type_util_js_1.isFixedTypedVector)(this.valueType)) {
      this._length = (0, value_type_util_js_1.fixedTypedVectorElementSize)(
        this.valueType,
      );
    } else if (
      this.valueType === value_type_js_1.ValueType.BLOB ||
      this.valueType === value_type_js_1.ValueType.MAP ||
      (0, value_type_util_js_1.isAVector)(this.valueType)
    ) {
      this._length = (0, reference_util_js_1.readUInt)(
        this.dataView,
        (0, reference_util_js_1.indirect)(
          this.dataView,
          this.offset,
      ...
      size = (0, reference_util_js_1.readUInt)(
        this.dataView,
  ...
  toObject() {
    ...
    if (this.isMap()) {
      const result = {};
      for (let i = 0; i < length; i++) {
        const key = (0, reference_util_js_1.keyForIndex)(
          i,
          this.dataView,
          this.offset,
          this.parentWidth,
