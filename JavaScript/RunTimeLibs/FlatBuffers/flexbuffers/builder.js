...
const stack_value_js_1 = require("./stack-value");
class Builder {
  constructor(
    size = 2048,
    dedupStrings = true,
  ...
  writeBlob(arrayBuffer) {
    ...
    this.stack.push(
      this.offsetStackValue(
        blobOffset,
        value_type_js_1.ValueType.BLOB,
        bitWidth,
      ),
    );
    this.offset = newOffset;
  ...
  writeString(str) {
    ...
    const stackValue = this.offsetStackValue(
      stringOffset,
      value_type_js_1.ValueType.STRING,
      bitWidth,
    );
  ...
  integrityCheckOnValueAddition() {
    ...
    if (
      ...
      if (
        this.stack[this.stack.length - 1].type !== value_type_js_1.ValueType.KEY
      ) {
  ...
  sort(stackPointer) {
    ...
    function shouldFlip(v1, v2) {
      if (
        v1.type !== value_type_js_1.ValueType.KEY ||
        v2.type !== value_type_js_1.ValueType.KEY
      ) {
  ...
  createVector(start, vecLength, step, keys = null) {
    ...
    for (let i = start; i < this.stack.length; i += step) {
      ...
      if (i === start) {
        ...
        typed =
          typed && (0, value_type_util_js_1.isTypedVectorElement)(vectorType);
      } else {
    ...
    const fix =
      typed &&
      (0, value_type_util_js_1.isNumber)(vectorType) &&
      vecLength >= 2 &&
      vecLength <= 4;
    if (keys !== null) {
    ...
    if (keys !== null) {
      return this.offsetStackValue(
        vecOffset,
        value_type_js_1.ValueType.MAP,
        bitWidth,
      );
    }
    if (typed) {
      const vType = (0, value_type_util_js_1.toTypedVector)(
        vectorType,
        fix ? vecLength : 0,
      );
    ...
    return this.offsetStackValue(
      vecOffset,
      value_type_js_1.ValueType.VECTOR,
      bitWidth,
    );
  }
  ...
  nullStackValue() {
    return new stack_value_js_1.StackValue(
      this,
      value_type_js_1.ValueType.NULL,
      bit_width_js_1.BitWidth.WIDTH8,
    );
  }
  ...
  boolStackValue(value) {
    return new stack_value_js_1.StackValue(
      this,
      value_type_js_1.ValueType.BOOL,
      bit_width_js_1.BitWidth.WIDTH8,
      value,
    );
  }
  ...
  intStackValue(value) {
    return new stack_value_js_1.StackValue(
      ...
      (0, bit_width_util_js_1.iwidth)(value),
      value,
    );
  }
  ...
  uintStackValue(value) {
    return new stack_value_js_1.StackValue(
      this,
      value_type_js_1.ValueType.UINT,
      (0, bit_width_util_js_1.uwidth)(value),
      value,
    );
  }
  ...
  floatStackValue(value) {
    return new stack_value_js_1.StackValue(
      this,
      value_type_js_1.ValueType.FLOAT,
      (0, bit_width_util_js_1.fwidth)(value),
      value,
    );
  }
  ...
  addInt(value, indirect = false, deduplicate = false) {
    ...
    const stackOffset = this.offsetStackValue(
      valueOffset,
      value_type_js_1.ValueType.INDIRECT_INT,
      stackValue.width,
    );
  ...
  addUInt(value, indirect = false, deduplicate = false) {
    ...
    const stackOffset = this.offsetStackValue(
      valueOffset,
      value_type_js_1.ValueType.INDIRECT_UINT,
      stackValue.width,
    );
    this.stack.push(stackOffset);
    this.offset = newOffset;
  ...
  addFloat(value, indirect = false, deduplicate = false) {
    ...
    const stackOffset = this.offsetStackValue(
      valueOffset,
      value_type_js_1.ValueType.INDIRECT_FLOAT,
      stackValue.width,
    );
    this.stack.push(stackOffset);
    this.offset = newOffset;
