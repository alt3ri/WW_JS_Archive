...
class ByteBuffer {
  ...
  readFloat64(offset) {
    ...
    utils_js_1.int32[utils_js_1.isLittleEndian ? 1 : 0] = this.readInt32(
      offset + 4,
    );
  ...
  getBufferIdentifier() {
    ...
    for (let i = 0; i < constants_js_1.FILE_IDENTIFIER_LENGTH; i++) {
      result += String.fromCharCode(
        this.readInt8(this.position_ + constants_js_1.SIZEOF_INT + i),
      );
    }
    return result;
  }
  ...
  __has_identifier(ident) {
    ...
    for (let i = 0; i < constants_js_1.FILE_IDENTIFIER_LENGTH; i++) {
      if (
        ident.charCodeAt(i) !=
        this.readInt8(this.position() + constants_js_1.SIZEOF_INT + i)
      ) {
        return false;
      }
    }
