...
class LevelExperienceComponent extends UiPanelBase_1.UiPanelBase {
  ...
  UpdateNextExp(i, s) {
    if (s >= this.CurrentMaxLevel) this.UpdateNextState(i, i, s);
    else {
      let t = this.MaxExpCacheMap.get(s);
      t || ((t = this.GetMaxExpFunction(s)), this.MaxExpCacheMap.set(s, t)),
        i >= t
  ...
  UpdateNextState(t, i, s) {
    this.PlayStartLevelUpSequence(s), this.SetNextLevel(s);
    i = t / i;
  ...
  SetNextLevelActive(t) {
    this.NextItem.SetUIActive(t),
      this.NextLevelArrow && this.NextLevelArrow.SetUIActive(t);
  }
  SetNextLevel(t) {
    this.SetNextLevelActive(!0), this.NextLevelText.SetText(t.toString());
  }
  SetMaxItemActive(t) {
  ...
  AddUpMaxExp(i) {
    let s = 0;
    for (let t = 1; t <= i; t++) s += this.StageMaxExp(t);
    return s;
  }
  GetMaxExp(t) {
  ...
  UpdateComponent(t, i, s, h = void 0) {
    (this.ArrivedLevel = t),
      (this.CurrentLevel = t),
