...
class AreaAudio {
  constructor() {
    (this.GHe = ""),
      ...
      (this.AreaChanged = () => {
        let t;
        ModelManager_1.ModelManager.AreaModel.AreaInfo.WuYinQuID > 0
          ? ((t =
  ...
  nje() {
    this.NHe
      ? this.OHe && this.FHe !== "battle_strong"
  ...
  _je() {
    if (this.jHe.size > 0) {
      for (const e of this.jHe) {
        const t = ModelManager_1.ModelManager.CharacterModel.GetHandle(e);
        t?.Valid &&
          EventSystem_1.EventSystem.RemoveWithTarget(
            t.Entity,
            EventDefine_1.EEventName.CharDamage,
  ...
  aje(t) {
    let e;
    const i = t.GetComponent(3)?.CreatureData;
