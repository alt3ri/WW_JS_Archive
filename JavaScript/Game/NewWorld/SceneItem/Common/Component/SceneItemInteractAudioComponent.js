...
let SceneItemInteractAudioComponent = class SceneItemInteractAudioComponent extends EntityComponent_1.EntityComponent {
      ...
      ? i.CollisionMaterial &&
        ...
        (t =
          ...
          : Log_1.Log.CheckError() &&
            Log_1.Log.Error("Audio", 33, "该类型未配置在 交互材质音频表 中", [
              "type",
              i,
            ]))
      : Log_1.Log.CheckError() &&
        Log_1.Log.Error("Audio", 33, "该组件未配置音效相关配置", [
          "PbDataId",
          e.GetPbDataId(),
        ]);
  }
    ...
    this.Pln && e - this.Pln < this.Aln
      ...
      : ((this.Pln = e),
        (e = t.Actor) !== this.n$t.Owner &&
          ...
          this.xln < this.bY
            ? Log_1.Log.CheckDebug() &&
              Log_1.Log.Debug("Audio", 33, "速度太小 不触发碰撞音效", [
                "Velocity",
                this.xln,
              ])
            : (this.Anr.MultiplyEqual(this.Mass),
              ...
              e instanceof TsBaseItem_1.default &&
                ...
                (this.wln = Vector_1.Vector.Create(
                  t.Entity.GetComponent(125).GetVelocity(),
                )),
                this.wln.MultiplyEqual(t.Entity.GetComponent(125).Mass)),
              Log_1.Log.CheckDebug() &&
                Log_1.Log.Debug(
                  "Audio",
                  33,
  ...
  GetVelocity() {
    ...
    return t?.IsValid()
      ? (this.Anr.FromUeVector(t.D_GetVelocity()), this.Anr)
