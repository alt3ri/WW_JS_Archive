"use strict";
let SceneItemReferenceComponent_1;
const __decorate =
...
const WALL_ONLY_MONSTER_COLLISION_NAME = new UE.FName(
  "InvisibleWallMonsterOnly",
);
const PLANEWIDTH =
  ...
  (SceneItemReferenceComponent_1 = class SceneItemReferenceComponent extends (
    ...
    constructor() {
      super(...arguments),
        ...
        (this.UMn = (e) => {
          let t = this.CMn.indexOf(e.toString());
          t >= 0 &&
            (this.CMn.splice(t, 1),
    ...
    ResetToInitState(e, t) {
      for (const s of this.Lo.ActorRefGroups) {
        let i = GameplayTagUtils_1.GameplayTagUtils.GetTagIdByName(e);
        if (i)
          if (s.EntityState === e)
            return void (s.Actions?.length > 0
    ...
    xMn() {
      if (
        ...
        const e = this.Hte.CreatureData.GetPbDataId();
        const t = StaticSceneUtils_1.StaticSceneUtils.GetActorRefByPbDataId(e);
        if (t) {
          for (const i of t)
    ...
    wMn() {
      ...
      for (const i of this.CMn) {
        const t = this.Lln.GetActor(new UE.FName(i));
        t
    ...
    AMn(e = !1) {
      for (const s of this.Lo.ActorRefGroups) {
        ...
        i &&
          this.Xte.HasTag(i) &&
          s.Actions?.length > 0 &&
          (!e && this.DMn.Size > 0 && this.DMn.Front === i
            ? this.DMn.Pop()
            : (Log_1.Log.CheckDebug() &&
                Log_1.Log.Debug(
                  "SceneItem",
    ...
    HandleSequence(t) {
      (t.LevelSequencePath && t.LevelSequencePath !== "None") ||
        (Log_1.Log.CheckError() &&
      ...
      (s = t.Intro
        ? t.Intro?.Type === 0
          ? ((e = t.Intro),
            new TransitStruct(
              0,
              e.Duration && e.Duration > 0 ? e.Duration : 0,
              0,
              0,
              !0,
            ))
          : ((e = t.Intro),
            new TransitStruct(
              1,
              e.Duration && e.Duration > 0 ? e.Duration : 0,
              e.FadeIn && e.FadeIn.Duration > 0 ? e.FadeIn.Duration : 1,
              e.FadeOut && e.FadeOut.Duration > 0 ? e.FadeOut.Duration : 1,
              !0,
              e.Mask,
            ))
          ...
          (e) => {
            e?.IsValid()
              ? (this.fMn
                ...
                t.PlayMode !== "direct" && t.PlayMode === "shortestPath"
    ...
    HandleAirWall(e) {
      ...
      for (const h of e.ActorRefs) {
        ...
        if (s.length < PATH_LENGTH)
          ...
          var s = s[1] + "." + s[2];
          let n = new UE.FName(s);
          const o = this.Lln.GetActor(n);
          if (o?.IsValid() && o instanceof UE.Brush) {
            const r = o.GetComponentByClass(
    ...
    OnChangeTimeDilation(e) {
      const t = this.Entity.GetComponent(107)?.CurrentTimeScale ?? 1;
